...
function redirect() { // VER SI SE PUEDE IMPLEMENTAR EN ROUTER/INDEX.JS
    if (page.value < 1)
        router.push({ name: "Files", params: { page: 1 } });
    if (totalPages.value > 0 && page.value > totalPages.value)
        router.push({ name: "Files", params: { page: totalPages.value } });
};
...
<template>
    <div class="table-container">
        <RouterLink class="btn btn-primary p-2 mb-3" :to="`/files/add`">+ Add File</RouterLink>
        <table class="table text-white p-2 text-center">
            ...
            <tbody>
                <tr v-for="f in files" :key="f.id">
                    ...
                    <td>
                        <RouterLink class="btn btn-success" :to="`/files/details/${f.id}`">
                        ...
                        <!-- <RouterLink class="btn btn-pencil" :to="`/files/update/${f.id}`">
